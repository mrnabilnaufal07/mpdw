...
testing<-data3[138:172,2] #data 138-172 merupakan 20% data selanjutnya
...
akurasi1.train
```
Akurasi pada model `Winter1 optimal` dengan data training lebih baik dibandingkan model `Winter1`. Ditandai dengan nilai SSE, MSE, dan RMSE yang lebih kecil. Namun, bukan bearti salah satu model tidak dapat digunakan. Oleh karena itu, akan dilihat juga akurasi dari data uji kedua model tersebut dengan tambahan `Nilai MAPE` sebagai tolak ukur kelayakan model.
...
akurasi1.test
```
Menurut `Lewis (1982)`, nilai `MAPE 20-50%` dapat diinterpretasikan sebagai model yang `Wajar`. Jika nilai `MAPE >= 50%` maka model dikatakan `tidak akurat`. Oleh karena itu baik model `Winter1` maupun `Winter1 optimal` masih bisa digunakan untuk prediksi.